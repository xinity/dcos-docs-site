...
| aws_template_storage_bucket | 包含在 [自定义高级 AWS 模板](/zh/1.11/installing/ent/cloud/aws/advanced/#create-your-templates) 中的 S3 bucket 的名称。 |
...
| ca_certificate_path | 使用此路径设置自定义 CA 证书。参见 [使用自定义 CA 证书](/zh/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) 页面，获取详细的配置参数参考。[enterprise type="inline" size="small" /] |
| ca_certificate_key_path | 使用此路径设置自定义 CA 证书。参见 [使用自定义 CA 证书](/zh/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) 页面，获取详细的配置参数参考。[enterprise type="inline" size="small" /] |
| ca_certificate_chain_path | 使用此路径设置自定义 CA 证书。参见 [使用自定义 CA 证书](/zh/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) 页面，获取详细的配置参数参考。[enterprise type="inline" size="small" /] |
...
如果您已经安装了群集，并且想要更改此设备，可以通过 [升级](/zh/1.11/installing/production/upgrading/) 实现，同时将 `adminrouter_tls_1_0_enabled` 参数设置为所需值。
...
如果您已经安装了群集，并且想要更改此设备，可以通过 [升级](/zh/1.11/installing/production/upgrading/) 实现，同时将 `adminrouter_tls_1_1_enabled` 参数设置为所需值。
...
如果您已经安装了群集，并且想要更改此设备，可以通过 [升级](/zh/1.11/installing/production/upgrading/) 实现，同时将 `adminrouter_tls_1_2_enabled` 参数设置为所需值。
...
如需更多信息，请参阅 [安全](/zh/1.11/security/ent/) 文档。
...
如需更多信息，请参阅 [示例](/zh/1.11/installing/ent/custom/configuration/examples/#docker-credentials)。
...
有关如何使用这些自定义检查的详细信息，请参阅 [示例](/zh/1.11/installing/ent/custom/configuration/examples/#custom-checks) 以及 [节点和群集运行状况检查](/zh/1.11/installing/ent/custom/node-cluster-health-check/) 文档。
...
如需更多信息，请参阅 [安全文档](/zh/1.11/security/ent/)。

## dcos_overlay_enable
指示是否启用 DC/OS 虚拟网络。

**注意：** 虚拟网络需要 Docker 1.11 或更高版本，但如果您使用 Docker 1.11 或更早版本，则必须指定 `dcos_overlay_enable: 'false'`。如需更多信息，请参阅 [系统要求](/zh/1.11/installing/ent/custom/system-requirements/)。
 ...
 **注意：** 最后三个字节必须为 `00`。
        ...
        *  `overlays`
 * `name` 典型名称（参见 [限制](/zh/1.11/networking/virtual-networks/)，了解有关命名虚拟网络的限制）。
 * `subnet` 分配给虚拟网络的子网。
 * `prefix` 分配给每个代理的子网大小，进而定义覆盖网络可运行的代理的数量。子网的大小在覆盖子网中创建。

 如需更多信息，请参阅 [示例](/zh/1.11/installing/ent/custom/configuration/examples/#overlay) 和 [文档](/zh/1.11/networking/virtual-networks/)。
...
如需更多信息，请参阅 [GPU 文档](/zh/1.11/deploying-services/gpu/)。

## gc_delay
清理执行器目录前等待的最长时间。建议您接受两天作为默认值。

## gpus_are_scarce
指示是否将 [GPU](/zh/1.11/deploying-services/gpu/) 作为群集中的稀缺资源。
...
负载均衡器的 DNS 名称或 IP 地址。指定后，将被作为对象替代名称纳入管理节点上 Admin Router 的 [DC/OS 证书](/zh/1.11/security/ent/tls-ssl/) 中。
...
默认为 `logrotate`。由于性能问题， 不推荐 `journald`。如需更多信息，请参阅 [记录 API](/zh/1.11/monitoring/logging/logging-api/#compatibility)。
...
- 如果没有要解析的内部主机名，您可以将其设置为 Google 或 AWS 等公共域名服务器。例如，可以指定 [Google 公共 DNS IP 地址 (Ipv4)](https://developers.google.com/speed/public-dns/docs/using)：
    ...
    - 8.8.8.8
    ```
- 如果没有 DNS 基础架构且没有权限访问互联网 DNS 服务器，您可以指定 `resolvers: []`。指定该设置后，发送到非 `.mesos` 的所有请求都将回复一个错误。如需更多信息，请参阅 Mesos-DNS [文档](/zh/1.11/networking/mesos-dns/)。
...
请查看外部持久卷 [文档](/zh/1.11/storage/external-storage/)，了解有关如何创建配置的信息。
...
请参阅 [安全模式](/zh/1.11/security/ent/#security-modes) 部分，了解对于每个参数的详细探讨。
...
带井号的超级用户密码。`superuser_password_hash` 通过使用安装工具 `--hash-password` 标记生成。第一个超级用户帐户用于提供登录 DC/OS 的方法，其中可添加其他管理帐户。如需更多信息，请参阅 [安全文档](/zh/1.11/security/ent/)。

## superuser_username（必填）[enterprise type="inline" size="small" /]
超级用户的用户名。此帐户使用 `superuser_password_hash`。如需更多信息，请参阅 [安全文档](/zh/1.11/security/ent/)。
...
如果已经安装了群集，并且希望禁用此功能，可以使用同一参数集进行 [升级](/zh/1.11/installing/production/upgrading/)。

## use_proxy
指示是否启用 DC/OS 代理。

* `use_proxy: 'false'` 请勿配置 DC/OS [组件](/zh/1.11/overview/architecture/components/) 以使用自定义代理。这是默认值。
* `use_proxy: 'true'` 请配置 DC/OS [组件](/zh/1.11/overview/architecture/components/) 以使用自定义代理。如果指定了 `use_proxy: 'true'`，您还可以指定这些参数：
...
如需更多信息，请参阅 [示例](/zh/1.11/installing/ent/custom/configuration/examples/#http-proxy)。
...
* `dcos_l4lb_enable_ipv6: 'false'` 请为 IPv6 容器禁用 [第 4 层负载均衡](/zh/1.11/networking/load-balancing-vips)。这是默认值。
