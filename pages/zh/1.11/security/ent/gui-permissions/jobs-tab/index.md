...
您可以授予用户访问 [**作业**屏幕](/zh/1.11/gui/jobs/) 的权限。新用户默认没有权限。

**注意：** 此过程授予访问**作业**屏幕及其内部所有作业的全部用户权限。如果您正在以 `strict` 或 `permissive` [安全模式](/zh/1.11/security/ent/#security-modes) 运行并希望配置细粒度用户访问权限，请参阅 [文档](/zh/1.11/deploying-services/service-groups/)。

# <a name="jobs-access-via-ui"></a>使用 Web 界面授予访问权限

**前提条件：**

- 不具有 `dcos:superuser` [权限](/zh/1.11/security/ent/users-groups/) 的 DC/OS 用户账户。
