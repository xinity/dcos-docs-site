...
本专题描述了如何部署具有独立角色、保留、配额和安全功能的非本地 Marathon 实例。高级非本地 Marathon 程序只能在您需要 [密钥](/zh/1.11/security/ent/secrets/) 或细粒度 ACL 时使用，否则请使用 [基本程序](/zh/1.11/deploying-services/marathon-on-marathon/basic/)。
...
- DC/OS 和 DC/OS CLI [已安装](/zh/1.11/installing/)。
- [DC/OS Enterprise CLI 0.4.14 或更高版本](/zh/1.11/cli/enterprise-cli/#ent-cli-install)。
- 自定义非本地 Marathon tarball。联系销售代表或 <sales@mesosphere.io> 获取本文件的访问权限。
- 每个专用 DC/OS 代理可以通过网络访问的专用 Docker 注册表。可以遵循 [以下](/zh/1.11/deploying-services/private-docker-registry/) 关于如何在 Marathon 中设置，或使用其他选项的说明（如 [DockerHub](https://hub.docker.com/)、[Amazon EC2 容器注册表](https://aws.amazon.com/ecr/)和 [Quay](https://quay.io/)）。
- 您必须以超级用户身份登录。
- 对群集的 SSH 访问。
...
1. [SSH](/zh/1.11/administering-clusters/sshcluster/) 到专用代理节点。
...
Marathon 服务账户可能是可选或必填项，具体取决于您的 [安全模式](/zh/1.11/security/ent/#security-modes)。
...
1. [SSH](/zh/1.11/administering-clusters/sshcluster/) 到包含 Docker 凭据文件的管理节点。

   ```bash
   dcos node ssh --master-proxy --mesos-id=<master-id>
   ```

1. 存储环境变量中每个专用代理的 IP 地址。<!-- What is this step for -->所需步骤取决于您的 [安全模式](/zh/1.11/security/ent/#security-modes)。
...
- 查看您的密钥，确保其包含正确的服务帐户 ID、私钥和 `login_endpoint` URL。如果是 `strict` 模式，应为 HTTPS，如果是 `disabled` 或 `permissive` 模式，则应为 HTTP。如果 URL 不正确，尝试 [升级 DC/OS Enterprise CLI](/zh/1.11/cli/enterprise-cli/#ent-cli-upgrade)，删除密钥，并重新创建。可以使用此命令查看内容：
...
所有 CLI 命令也可通过 [IAM API](/zh/1.11/security/ent/iam-api) 执行。
...
1. 选择要授予权限的用户名或组名。
 ...
 ![添加 cory 权限](/zh/1.11/img/services-tab-user.png)
...
1. 单击 **INSERT PERMISSION STRING** 以切换对话框。
 ...
 ![添加权限](/zh/1.11/img/services-tab-user3.png)
...
1. 输入您的用户名和密码，然后单击 **登录**。
 ...
 ![Log in DC/OS](/zh/1.11/img/gui-installer-login-ee.gif)

 图 4. 成功了！

 ![Marathon on Marathon](/zh/1.11/img/mom-marathon-gui.png)
