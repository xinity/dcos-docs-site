...
- **Admin Router 管理节点** 公开 [管理节点路由](/zh/1.11/api/master-routes/)。

 此配置在每个管理节点上运行，用作与 DC/OS 组件交互的主 API 网关。

- **Admin Router 代理节点** 公开 [代理路由](/zh/1.11/api/agent-routes/)。
...
有关版本控制机制的详细信息，请参阅[版本控制](/zh/1.11/api/versioning/)。
...
有关如何获取和使用认证令牌的详细信息，请参阅 [验证 HTTP API 端点](/zh/1.11/security/ent/iam-api/)。
...
- **粗粒度权限** 是在路由级别 [由 Admin Router 执行](/zh/1.11/security/ent/perms-reference/#admin-router)。
- **细粒度权限** 由单个后端组件服务执行。

[权限管理](/zh/1.11/security/ent/perms-management/)可通过具有 [超级用户权限](/zh/1.11/security/ent/perms-reference/#superuser) 的用户使用 [身份和访问管理 API](/zh/1.11/security/ent/iam-api/)执行。具有超级用户权限的用户也有隐式权限访问所有路由。
