...
The [DC/OS graphical user interface (GUI)](/dcos/1.11/gui/) is an interface for remotely controlling and managing a DC/OS cluster from a web browser. The GUI is also sometimes called the DC/OS UI or DC/OS web interface.

## <a name="dcos-cli"></a>DC/OS CLI

The [DC/OS command line interface (CLI)](/dcos/1.11/cli/) is an interface for remotely controlling and managing a DC/OS cluster from a terminal.
...
For more information, see [Network Security](/dcos/1.11/administering-clusters/) and [Adding Agent Nodes](/dcos/1.11/administering-clusters/add-a-node/).
...
- Public agent nodes are used primarily for externally facing reverse proxy load balancers, like [Marathon-LB](/dcos/services/marathon-lb/1.12/).
...
For more information, see [Converting Agent Node Types](/dcos/1.11/administering-clusters/convert-agent-type/).
...
- One method of allowing master nodes to change IPs involves running ZooKeeper with Exhibitor on the bootstrap machine. Other alternatives include using S3, DNS, or static IPs, with various tradeoffs. For more information, see [configuring the exhibitor storage backend](/dcos/1.11/installing/production/advanced-configuration/configuration-reference/#exhibitor-storage-backend).
- If a bootstrap machine is not required for managing master node IP changes or as an SSH jumpbox, it can be shut down after bootstrapping and spun up on demand to [add new nodes](/dcos/1.11/administering-clusters/add-a-node/) to the cluster.

For more information, see the [system requirements](/dcos/1.11/installing/production/system-requirements/#bootstrap-node).
...
For more information, see [Cloud Installation Options](/dcos/1.11/installing/evaluation/cloud-installation/).
...
For more information, see the [Mesos-DNS documentation](/dcos/1.11/networking/DNS/mesos-dns/).
