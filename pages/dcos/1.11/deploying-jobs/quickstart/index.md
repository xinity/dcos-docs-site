...
**Note:** The DC/OS web interface provides a subset of the CLI and API functionality. For advanced job configurations, use the [dcos job](/dcos/1.11/cli/command-reference/dcos-job/) commands or the Jobs [API](#jobs-api).
...
![Create JOB UI](/dcos/1.11/img/create-job.png)
...
### **Labels**
**Label Name** and **Label Value** - Attach metadata to your jobs so you can filter them. [Learn more about labels](/dcos/1.11/tutorials/task-labels/).

## Job Groups
You can add a job to a an existing job group or create one when you create the job. Use dots in your job ID to nest the job in a group. For instance, if you enter job ID `marketing.myjob`, `myjob` will be created in the `marketing` group. In DC/OS Enterprise, you can [use job groups](/dcos/1.11/deploying-jobs/job-groups/) to implement fine-grained user access.
