...
Logs are useful tools for seeing events and the conditions that occurred before they emerged. Often logs include error messages that can supply helpful information regarding the cause of the error. As logging is an important topic in its own right, we recommend the [DC/OS logging documentation](/dcos/1.11/monitoring/logging/#system-logs), for more information.
...
**Note:** Need a scalable way to manage and search your logs? It could be worth building an [ELK stack for log aggregation and filtering](/dcos/1.11/monitoring/logging/aggregating/filter-elk/).
