...
# Prerequisites
* A [running DC/OS cluster](/dcos/1.11/tutorials/dcos-101/cli/) with [the DC/OS CLI installed](/dcos/1.11/tutorials/dcos-101/cli/).
* [app1](/dcos/1.11/tutorials/dcos-101/app1/) deployed and running in your cluster.
...
# Service Discovery
  [Service discovery](/dcos/1.11/networking/) enables addressing of applications independently of where they are running in the cluster, which is particularly useful in cases where applications may fail and be restarted on a different host.
...
# Mesos-DNS
  ...
  [Mesos-DNS](/dcos/1.11/networking/DNS/mesos-dns/) assigns DNS entries for every task, which are resolvable from any node in the cluster. The naming pattern for these entries is  *task.scheduler.mesos*

  The default scheduler for jobs is [Marathon](/dcos/1.11/overview/architecture/components/#marathon), so the Mesos-DNS name for your Redis service is *redis.marathon.mesos*.
