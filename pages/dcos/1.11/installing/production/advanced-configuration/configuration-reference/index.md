...
|----------------------------------------|-----------------------------------------------------------------------------------------------------------------------------------------------------------|
| [agent_list](#agent-list)                                              | A YAML nested list (`-`) of IPv4 addresses to your [private agent](/dcos/1.11/overview/concepts/#private-agent-node) host names. |
...
| [aws_template_storage_bucket](#aws-template-storage-bucket)                | The name of an S3 bucket to contain [customized advanced AWS templates](/dcos/1.11/installing/evaluation/community-supported-methods/aws/advanced/#create-your-templates). |
...
| [public_agent_list](#public-agent-list)                               | A YAML nested list (`-`) of IPv4 addresses to your [public agent](/dcos/1.11/overview/concepts/#public-agent-node) host names.  |
...
| [ca_certificate_path](#ca-certificate-path-enterprise)                   | Use this to set up a custom CA certificate. See [using a Custom CA Certificate](/dcos/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) page for a detailed configuration parameter reference. [enterprise type="inline" size="small" /] |
| [ca_certificate_key_path](#ca-certificate-key-path-enterprise)           | Use this to set up a custom CA certificate. See [using a Custom CA Certificate](/dcos/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) page for a detailed configuration parameter reference. [enterprise type="inline" size="small" /] |
...
If you have already installed your cluster and would like to change this in place, you can go through an [upgrade](/dcos/1.11/installing/production/upgrading/) with the `adminrouter_tls_1_0_enabled` parameter set to the desired value.
...
If you have already installed your cluster and would like to change this in-place, you can go through an [upgrade](/dcos/1.11/installing/production/upgrading/) with the `adminrouter_tls_1_1_enabled` parameter set to the desired value.
...
If you have already installed your cluster and would like to change this in place, you can go through an [upgrade](/dcos/1.11/installing/production/upgrading/) with the `adminrouter_tls_1_2_enabled` parameter set to the desired value.
...
### agent_list
A YAML nested list (`-`) of IPv4 addresses to your [private agent](/dcos/1.11/overview/concepts/#private-agent-node) host names.
...
The name of an S3 bucket to contain [customized advanced AWS templates](/dcos/1.11/installing/evaluation/community-supported-methods/aws/advanced/#create-your-templates).
...
For more information, see the [security](/dcos/1.11/security/ent/) documentation.

### ca_certificate_path   [enterprise type="inline" size="small" /]              

Use this to set up a custom CA certificate. See [using a Custom CA Certificate](/dcos/1.11/security/ent/tls-ssl/ca-custom#configuration-parameter-reference) page for a detailed configuration parameter reference. 
...
For more information, see the [examples](/dcos/1.11/installing/production/deploying-dcos/configuration/examples/#docker-credentials).
...
For more information on how these custom checks are used, see the [examples](/dcos/1.11/installing/production/deploying-dcos/configuration/examples/#custom-checks) and [Node and Cluster Health Check](/dcos/1.11/installing/production/deploying-dcos/node-cluster-health-check/) documentation.
...
For more information, see the [security documentation](/dcos/1.11/security/ent/).

### dcos_l4lb_enable_ipv6
Indicates whether layer-4 load-balancing is available for IPv6 containers.
*  `dcos_l4lb_enable_ipv6: 'false'` Disables [layer-4 load balancing](/dcos/1.11/networking/load-balancing-vips/) for IPv6 containers. This is the default value.
...
For more information, see the [system requirements](/dcos/1.11/installing/production/system-requirements/).
...
*  `dcos_overlay_enable: 'true'` Enable the DC/OS virtual network. This is the default value. After the virtual network is enabled, you can also specify the following parameters:
    ...
    *  `dcos_overlay_network` This group of parameters defines a virtual network for DC/OS.  The default configuration of DC/OS provides a virtual network named `dcos` with this YAML configuration:
        ...
        *  `overlays`
            *  `name` The canonical name (see [limitations](/dcos/1.11/networking/SDN/dcos-overlay/) for constraints on naming virtual networks).
...
For more information, see the [example](/dcos/1.11/installing/production/deploying-dcos/configuration/examples/#overlay) and [documentation](/dcos/1.11/networking/SDN/dcos-overlay/)
...
*  `enable_docker_gc: 'true'` Run the docker-gc scripts once every hour. This is the default value for [cloud](/dcos/1.11/installing/evaluation/) template installations.
*  `enable_docker_gc: 'false'` Do not run the docker-gc scripts once every hour. This is the default value for [custom](/dcos/1.11/installing/production/) installations.
...
For more information, see the [GPU documentation](/dcos/1.11/deploying-services/gpu/).
...
### gpus_are_scarce
Indicates whether to treat [GPUs](/dcos/1.11/deploying-services/gpu/) as a scarce resource in the cluster.
...
The DNS name or IP address for the load balancer. If specified, this is included as subject alternative name in the [DC/OS certificate](/dcos/1.11/security/ent/tls-ssl/) of the Admin Router on the master nodes.
...
The default is `logrotate`. Due to performance issues, `journald` is not recommended. For details, see [Logging API](/dcos/1.11/monitoring/logging/logging-api/#compatibility).
...
### public_agent_list
A YAML nested list (`-`) of IPv4 addresses to your [public agent](/dcos/1.11/overview/concepts/#public-agent-node) host names.
...
-  If you do not have internal hostnames to resolve, you can set this to a public nameserver like Google or AWS. For example, you can specify the [Google Public DNS IP addresses (IPv4)](https://developers.google.com/speed/public-dns/docs/using):
    ...
    - 8.8.8.8
    ```
-  If you do not have a DNS infrastructure and do not have access to internet DNS servers, you can specify `resolvers: []`. By specifying this setting, all requests to non-`.mesos` will return an error. For more information, see the Mesos-DNS [documentation](/dcos/1.11/networking/mesos-dns/).
...
See the external persistent volumes [documentation](/dcos/1.11/storage/external-storage/) for information on how to create your configuration.
...
Refer to the [security modes](/dcos/1.11/security/ent/#security-modes) section for a detailed discussion of each parameter.
...
(Required) The user name of the superuser. This account uses the `superuser_password_hash`. For more information, see the [security documentation](/dcos/1.11/security/ent/).
...
If you have already installed your cluster and would like to disable this in place, you can go through an [upgrade](/dcos/1.11/installing/production/upgrading/) with the same parameter set.

### use_proxy
Indicates whether to enable the DC/OS proxy.

*  `use_proxy: 'false'` Do not configure DC/OS [components](/dcos/1.11/overview/architecture/components/) to use a custom proxy. This is the default value.
*  `use_proxy: 'true'` Configure DC/OS [components](/dcos/1.11/overview/architecture/components/) to use a custom proxy. If you specify `use_proxy: 'true'`, you can also specify these parameters:
    ...
    **Note:** Wildcard characters (`*`) are not supported.</p>
    ...
    For more information, see the [examples](/dcos/1.11/installing/ent/custom/configuration/examples/#http-proxy).
