...
If your node has gone down in an unplanned way, you only have to [Decommission the node](/dcos/1.11/administering-clusters/delete-node/#decommission-the-node/).
...
- If you are working with stateful services, like the [DC/OS data services](/dcos/services/). It is expensive for stateful services to reschedule tasks, so the services need to know that an agent is not going to come back online before they reschedule.
...
1. [SSH to the agent node](/dcos/1.11/administering-clusters/sshcluster/) you wish to shut down.
