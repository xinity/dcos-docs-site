...
For example, the [Marathon component](/dcos/1.11/overview/architecture/components/#marathon) serves the [Marathon API](/dcos/1.11/deploying-services/marathon-api/) under the route `/service/marathon` and one of its resource paths is `/v2/apps`, so the full path to that resource is `/service/marathon/v2/apps`.
