...
The default Linux user of a service or job can vary according to the security mode and the container type. See [Linux users](/dcos/1.11/security/ent/#linux-users) for more information.
...
Many Universe services ignore overrides of their user accounts except in `strict` mode. We provide detailed steps for overriding the default Linux user for services that support this in [Service Accounts](/dcos/1.11/security/ent/service-auth/). Refer to the section that pertains to the service of interest for step-by-step instructions. The procedures also include how to configure the service to use encryption and service accounts.

Remember to grant permission to perform the `create` action on the `dcos:mesos:master:task:user[:<linux-user-name>]` resource to the service account user that the Universe service is launched with. See [Mesos Permissions](/dcos/1.11/security/ent/perms-reference/#mesos-permissions) for more information.

# <a name="marathon-app-def"></a>Overriding the default Linux user via Marathon app definition

Marathon app definitions provide a `"user"` key which you can use to override the default Linux user. **Tip:** Reference the [Marathon documentation](/dcos/1.11/deploying-services/creating-services/) for more details on writing Marathon services.
...
- You have installed and are logged into the [DC/OS CLI](/dcos/1.11/cli/).
...
1. Deploy the service using the [Marathon API](/dcos/1.11/deploying-services/marathon-api/).
...
**Note:** Refer to the [Jobs documentation](/dcos/1.11/deploying-jobs/quickstart/) for more information about creating and deploying jobs.
