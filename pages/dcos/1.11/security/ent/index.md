...
### Admin Router
Access to the admin zone is controlled by the [Admin Router](/dcos/1.11/overview/architecture/components/#admin-router).
...
![Security Zones](/dcos/1.11/img/security-zones.png)
...
See the [permissions reference](/dcos/1.11/security/ent/perms-reference/) for a complete description.
...
- [SSH](/dcos/1.11/administering-clusters/sshcluster/) into your master and view the contents of `/opt/mesosphere/etc/bootstrap-config.json`.

# <a name="authentication"></a>Authentication
All requests from outside of the DC/OS cluster require an authentication token. Depending on your security mode, in-cluster authentication tokens may be required. For more information, see the [Service Accounts documentation](/dcos/1.11/security/ent/service-auth/).
...
![User authentication](/dcos/1.11/img/authn-user.png)
...
![Authorization sequence](/dcos/1.11/img/authz.png)
...
For more information about permissions, refer to [Managing permissions](/dcos/1.11/security/ent/perms-reference/).

# <a name="encryption"></a>Transport Layer Security (TLS) Encryption

The encryption of DC/OS communications varies according to your [security mode](/dcos/1.11/security/ent/#security-modes).
...
For more information, see [Securing communication with TLS](/dcos/1.11/security/ent/tls-ssl/).
...
In `strict` and `permissive` security modes, you can use [permissions](/dcos/1.11/security/ent/perms-reference/#marathon-metronome) to restrict user's access on a per service/job or service/job group basis.

To learn how to do this, see [Controlling user access to services](/dcos/1.11/deploying-services/service-groups/) and [Controlling user access to jobs](/dcos/1.11/deploying-jobs/job-groups/).
...
- **User access to secrets:** use [permissions](/dcos/1.11/security/ent/perms-reference/#secrets) to control which users can access what secrets and what operations they can perform.

- **Application access to secrets:** use [spaces](/dcos/1.11//security/ent/#spaces) to control which applications can retrieve what secrets.

# <a name="linux-users"></a>Linux User Accounts

The default Linux user for tasks and sandbox files varies according to your [security mode](/dcos/1.11/security/ent/#security-modes) and the [type of container](/dcos/1.11/deploying-services/containerizers/) the task runs inside of.

By default, all tasks will run inside of Docker containers. Please see [Deploying a Docker-based Service to Marathon](/dcos/1.11/deploying-services/creating-services/deploy-docker-app/) for an example.
...
See [Overriding the default Linux user](/dcos/1.11/security/ent/users-groups/config-linux-user/).
