...
![DC/OS Node Types](/dcos/1.12/img/node-types-redesigned.png)
...
A public agent node is an agent node that is on a network that allows ingress from outside of the cluster via the cluster’s [infrastructure networking](/dcos/1.12/overview/concepts/#infrastructure-network).
...
A private agent node is an agent node that is on a network that does not allow access from outside of the cluster via the cluster’s [infrastructure networking](/dcos/1.12/overview/concepts/#infrastructure-network).
...
Because these resources are undifferentiated, most tasks are scheduled on private agent nodes and are inaccessible from outside the cluster, decreasing the surface area that could be accessed by malicious attackers. For this reason, clusters are generally comprised of mostly private agent nodes. Likewise, most [Mesosphere Universe](/dcos/1.12/overview/concepts/#mesosphere-universe) packages install by default on private agent nodes.

## More Information

For more on master and agent node components, see [Components](/dcos/1.12/overview/architecture/components/).

For more on security, see [Securing your cluster](/dcos/1.12/administering-clusters/securing-your-cluster/).

For more on scaling your cluster, see [Adding agent nodes](/dcos/1.12/administering-clusters/add-a-node/).

For more on configuring public nodes, see [Converting agent node types](/dcos/1.12/administering-clusters/convert-agent-type/).
