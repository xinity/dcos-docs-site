...
- To update to a newer maintenance version (for example, 1.11.6 to 1.11.8), refer to the instructions for [patching](/dcos/1.12/installing/production/patching/).
- To modify the cluster configuration, refer to the instructions for [patching](/dcos/1.12/installing/production/patching/).
...
- The Production installation method is the **only** recommended upgrade path for DC/OS. It is recommended that you familiarize yourself with the [DC/OS Deployment Guide](/dcos/1.12/installing/production/deploying-dcos/) before proceeding.
- Review the [release notes](/dcos/1.12/release-notes/) before upgrading DC/OS.
...
- Take a [snapshot of ZooKeeper](/dcos/1.12/installing/installation-faq/#q-how-do-i-backup-zookeeper-using-guano) prior to upgrading. Marathon supports rollbacks, but does not support downgrades.
- Take a [snapshot of the IAM database](/dcos/1.12/installing/installation-faq/#q-how-do-i-backup-the-iam-database-enterprise) prior to upgrading. [enterprise type="inline" size="small" /]
...
- [Back up your cluster](/dcos/1.12/administering-clusters/backup-and-restore/). [enterprise type="inline" size="small" /]
- Optional: You can add custom [node and cluster health checks](/dcos/1.12/installing/production/deploying-dcos/node-cluster-health-check) to your `config.yaml`.
...
2.  Merge the old `config.yaml` into the new `config.yaml` format. In most cases the differences will be minimal.
    ...
    *  The syntax of the `config.yaml` file can be different from the earlier version. For a detailed description of the current `config.yaml` syntax and parameters, see the [advanced configuration reference](/dcos/1.12/installing/production/advanced-configuration/configuration-reference/).
...
1.  Copy and update the DC/OS 1.10 `config.yaml` and `ip-detect` files to a new, clean folder on your bootstrap node.
    ...
    *  You cannot change the `exhibitor_zk_backend` setting during an upgrade.
    *  The syntax of the DC/OS 1.12 `config.yaml` file differs from that of previous versions. For a detailed description of the current `config.yaml` syntax and parameters, see the [advanced configuration reference](/dcos/1.12/installing/production/advanced-configuration/configuration-reference/).
...
1.  Validate the upgrade:
    ...
    1.  Verify that `/opt/mesosphere/bin/mesos-master --version` indicates that the upgraded master is running the version of Mesos specified in the [release notes](/dcos/1.12/release-notes/), for example `1.5.1`.
...
If your upgrade fails because of a [custom node or cluster check](/dcos/1.12/installing/production/deploying-dcos/node-cluster-health-check/), run these commands for more details:
```bash
