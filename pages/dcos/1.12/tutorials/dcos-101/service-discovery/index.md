...
# Service Discovery
  [Service discovery](/dcos/1.12/networking/) enables addressing of applications independently of where they are running in the cluster, which is particularly useful in cases where applications may fail and be restarted on a different host.
...
# Mesos-DNS
  ...
  [Mesos-DNS](/dcos/1.12/networking/DNS/mesos-dns/) assigns DNS entries for every task, which are resolvable from any node in the cluster. The naming pattern for these entries is `task.scheduler.mesos`.

  The default scheduler for jobs is [Marathon](/dcos/1.12/overview/architecture/components/#marathon), so the Mesos-DNS name for your Redis service is **redis.marathon.mesos**.
...
# Named Virtual IPs
  ...
  * [Named VIPs](/dcos/1.12/networking/load-balancing-vips/) allow you to assign name/port pairs to your apps, which means you can give your apps meaningful names with a predictable port. They also provide built-in load balancing when using multiple instances of an application.
...
In [the next section](/dcos/1.12/tutorials/dcos-101/app2/), you will deploy an app which provides a GUI to users.