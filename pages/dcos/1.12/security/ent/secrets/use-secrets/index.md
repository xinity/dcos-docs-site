...
- An existing secret. The examples below use a secret called `my-secret` stored in the `developer` path. If you complete the steps in [Creating secrets](/dcos/1.12/security/ent/secrets/create-secrets/), you will meet this prerequisite.

- [DC/OS CLI installed](/dcos/1.12/cli/install/) and the [DC/OS Enterprise CLI installed](/dcos/1.12/cli/enterprise-cli/#ent-cli-install).
- You must [get the root cert](/dcos/1.12/security/ent/tls-ssl/get-cert/) before issuing the curl commands in this section.  
- The appropriate permissions for your [security mode](/dcos/1.12/security/ent/#security-modes).
  ...
  As long as the path of the secret and the path of the group [match up properly](/dcos/1.12/security/ent/#spaces), the service will be able to access the secret value.

The procedure differs depending on whether or not you want to make the secret available to a [pod](/dcos/1.12/deploying-services/pods/) or to an individual service.
...
1. Log into the web interface as a user with the necessary permissions as discussed in [Permissions Management](/dcos/1.12/security/ent/perms-management/) and [Granting Access to the Secrets Tab](/dcos/1.12/security/ent/gui-permissions/secrets-tab/).

1. Click the **Services** tab.

1. Click the **+** icon in the top right.

    ![Add a Service](/dcos/1.12/img/add-service.png)
...
1. Copy one of the following simple application definitions and paste it into the black box. This application definition creates a new service inside of the developer group and references a secret stored inside a developer path.
   ...
   Because the service and the secret paths match, the service will be able to access the secret. See [Spaces](/dcos/1.12/security/ent/#spaces) for more details about the paths.
...
1. Within a text editor, create an application definition for your Marathon service. The following application definition creates a new service inside of the developer group and references a secret stored inside a developer path.
   ...
   Because the service group and the secret paths match, the service will be able to access the secret. See [Spaces](/dcos/1.12/security/ent/#spaces) for more details about the paths.
