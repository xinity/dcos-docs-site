...
服务或作业的默认 Linux 用户可能因安全模式和容器类型而异 请参阅 [Linux 用户](/dcos/cn/1.12/security/ent/#linux-users) 了解更多信息。
...
许多 Universe 服务忽略其用户帐户的覆盖，在 `strict` 模式下除外。我们提供支持在服务账户[服务账户](/dcos/cn/1.12/security/ent/service-auth/)中覆盖的服务默认 Linux 用户的详细覆盖步骤。有关逐步说明，请参阅与感兴趣的服务相关的部分。程序还包括如何配置服务以使用加密和服务帐户。
...
Marathon 应用定义提供用于覆盖默认 Linux 用户的 `"user"` 密钥。**提示：**参考 [Marathon 文档](/dcos/cn/1.12/deploying-services/creating-services/)，了解有关编写 Marathon 服务的更多详细信息。

以下教程将展示所有权如何运作。在开始之前，请确保：

- 代理程序上已存在 Linux 用户帐户。
- 您已安装并登录到 [DC/OS CLI](/dcos/cn/1.12/cli/)。
- 必须遵守 [下载根证书](/dcos/cn/1.12/security/ent/tls-ssl/get-cert/) 中的步骤才能发布此部分的 curl 命令。
...
1. 使用 [Marathon API](/dcos/cn/1.12/deploying-services/marathon-api/) 部署服务。
