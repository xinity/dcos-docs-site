...
- 您必须熟悉 [DC/OS 安装指南][安装](/dcos/cn/1.12/installing/production/deploying-dcos/installation/#custom-build-file)。
...
5. 构建安装工具包。
 ...
 4. 使用 Docker [运行](/dcos/cn/1.12/installing/production/deploying-dcos/installation/#nginx-cmd) 命令运行 nginx 容器以提供 [安装文件](/dcos/cn/1.12/installing/production/deploying-dcos/installation/#custom-build-file)。
...
1. 将 1.11`config.yaml` 转换为 1.12 `config.yaml` 格式后，您可以构建安装工具包：
 ...
 1. 使用 Docker [运行](/dcos/cn/1.12/installing/production/deploying-dcos/installation/#nginx-cmd) 命令运行 nginx 容器以提供 [安装文件] [安装](/dcos/cn/1.12/installing/production/deploying-dcos/installation/#custom-build-file)。
...
如果您的升级因为 [自定义节点或群集检查](/dcos/cn/1.12/installing/ent/custom/node-cluster-health-check/#custom-health-checks) 而失败，运行以下命令可了解更多详细信息：
```bash
