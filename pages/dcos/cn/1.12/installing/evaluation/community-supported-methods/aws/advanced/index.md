...
- 符合 bootstrap 节点 [系统要求](/dcos/cn/1.12/installing/ent/custom/system-requirements/) 的节点。
...
1. 在 `genconf` 目录中创建配置文件并另存为 `config.yaml`。此配置文件指定您的 AWS 凭据以及用于存储生成的原图的 S3 位置。
    ...
    有关参数描述和配置示例，请参阅 [文档](/dcos/cn/1.12/installing/ent/custom/configuration/configuration-parameters/)。
...
1. 转到 [CloudFormation](https://console.aws.amazon.com/cloudformation/home) 并单击 **创建堆栈**。
    ...
    - 在 **选择模板** 页面，从您的工作站上传 [Zen](/dcos/cn/1.12/installing/evaluation/aws/) 模板，然后单击 **下一步**。
...
1. 在 **指定详情**页面指定这些值，然后单击 **下一步**。
    ...
    * **堆栈名称** 指定群集名称。
    * **CustomAMI** 可选：指定 AMI ID。如需更多信息，请参阅 [使用自定义 AMI 进行安装](/dcos/cn/1.12/installing/production/system-requirements/custom-ami/)。
...
可以通过创建新的堆栈来添加更多代理节点。使用 [advanced-priv-agent.json](/dcos/cn/1.12/installing/evaluation/aws/) 或 [advanced-pub-agent.json](/dcos/cn/1.12/installing/evaluation/aws/) 模板。这些模板创建代理，然后作为 AutoScalingGroup 的一部分附加到 `PrivateAgentStack` 或 `PublicAgentStack`。
...
对于所有高级配置选项，请参阅模板参考 [文档](/dcos/cn/1.12/installing/evaluation/aws/)。
...
# 模板参考
如需完整的高级配置选项，请参阅模板参考 [文档](/dcos/cn/1.12/installing/evaluation/aws/)。
