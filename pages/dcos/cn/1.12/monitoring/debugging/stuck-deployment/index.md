...
由于 Mesos 资源邀约无法匹配来自服务或 Pod [Marathon 应用定义](/dcos/cn/1.12/deploying-services/creating-services/) 的资源请求，DC/OS 服务或 Pod 可能无法部署。
...
- Marathon 没有获得为启动应用程序所需的资源邀约。
 使用 [CLI](/dcos/cn/1.12/monitoring/debugging/cli-debugging/) 调试子命令或 [DC/OS Web 界面中的调试页面](/dcos/cn/1.12/monitoring/debugging/gui-debugging/) 来解决来自 Mesos 的不匹配或未接受的资源邀约。您也可以 [查阅服务和任务日志](/dcos/cn/1.12/monitoring/logging/)。

- 服务的运行状况检查失败。如果服务执行运行状况检查，则在通过运行状况检查前，部署不会完成。您可以从 [DC/OS Web 界面](/dcos/cn/1.12/monitoring/debugging/gui-debugging/) 查看执行 Marathon 运行状况检查的服务的运行状况。要通过 Marathon 运行状况检查了解有关服务运行状况的更多信息，请从 DC/OS CLI 运行 `dcos marathon app list --json`。
