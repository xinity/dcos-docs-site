...
- [DC/OS CA](/dcos/cn/1.11/security/ent/tls-ssl/) - 集中化证书授权
 (CA) 用于验证和最终签署证书签名请求 (CSR)。
- [DC/OS 密码](/dcos/cn/1.11/security/ent/secrets/) - 一种向包组件分配
 TLS 工件的集中、安全方式，例如，Kubernetes 组件和
 在同一 DC/OS 集群中的其他应用程序。
- [DC/OS 服务帐户](/dcos/cn/1.11/security/ent/service-auth/) - 我们的包和
 应用程序所需，以对上述服务进行身份认证。
...
为了在设置 TLS 时充分利用 DC/OS Enterprise PKI 基础架构，需要有权限管理 CA 和密码的 [服务账户](/dcos/cn/1.11/security/ent/service-auth/)。在安装 Kubernetes 包之前， **必须** 进行配置。

为了配置此类服务帐户，首先需要安装 [DC/OS Enterprise CLI](/dcos/cn/1.11/cli/enterprise-cli/)。然后，执行以下操作：
...
下一步，您需要 [授予](/dcos/cn/1.11/security/ent/perms-management/) 服务帐户正确的权限。
...
ingress 控制器。请参阅
[Ingress](/dcos/cn/services/kubernetes/1.2.1-1.10.6/ingress/)
文档，以了解有关在您的
