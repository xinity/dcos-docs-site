...
本主题介绍如何为 Kafka 配置 DC/OS 访问权限。根据您的 [安全模式](/dcos/cn/1.11/security/ent/#security-modes/)，Kafka 需要 [服务认证](/dcos/cn/1.11/security/ent/service-auth/) 才能访问 DC/OS。
...
- 如果您的 [安全模式](/latest/security/ent/#security-modes/) 是 `permissive` 或 `strict`，您必须 [获取 root 证书](/dcos/cn/1.11/networking/tls-ssl/get-cert/)，才能发出本部分中的 curl 命令。
