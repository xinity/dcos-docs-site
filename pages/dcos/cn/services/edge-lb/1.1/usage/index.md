...
本页面介绍 Edge-LB 使用的常见命令。有关 CLI 命令的更详细列表，请参阅 [dcos edgelb cli 参考](/dcos/cn/services/edge-lb/1.1/cli-reference/)。

# 先决条件

- Edge-LB [安装和运行](/dcos/cn/services/edge-lb/1.1/installing/)。

# 创建池

启动服务并创建 [池配置文件](/dcos/cn/services/edge-lb/1.1/pool-configuration/)后，您可以使用 CLI 来部署它：
...
此重新加载的属性能够实现
[蓝色/绿色部署这样的策略](/dcos/cn/services/edge-lb/1.1/tutorials/blue-green-deploy/)。
...
有关 Edge-LB 命令的列表，请参阅 [CLI 参考](/dcos/cn/services/edge-lb/1.1/cli-reference/) 页面。
