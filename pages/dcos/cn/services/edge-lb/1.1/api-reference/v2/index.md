...
要确定您集群的地址，请参阅 [集群访问](/dcos/cn/1.11/api/access/)。
...
要验证 API 请求，请参阅 [获取身份认证令牌](/dcos/cn/1.11/security/ent/iam-api/#obtaining-an-authentication-token) 和 [传递身份认证令牌](/cn1.11/security/ent/iam-api/#pass-an-authentication-token)。
...
要为您的帐户分配权限，请参阅 [分配权限](/dcos/cn/1.11/security/ent/perms-reference/)。

# 通用资源

这些基层路由与 V2 一起添加，可用于从 V1 或 V2 池配置读取数据。

[swagger api='/dcos/services/api/edge-lb-common.yaml']

# V2 资源

以上两条路由均提供以下资源：

[swagger api='/dcos/services/api/edge-lb-v2.yaml']
