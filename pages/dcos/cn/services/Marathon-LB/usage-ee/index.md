...
安装流程因您的 [安全模式而异](/dcos/cn/1.11/installing/ent/custom/configuration/configuration-parameters/#security-enterprise)。有关分步说明，请参阅与安全模式相对应的部分。
...
- 作为具有 [必要权限 的用户通过 `dcos auth login` 登录](/dcos/cn/1.11/security/ent/perms-reference/)。
...
要在 `disabled` 和 `permissive` 模式中从目录中安装 Marathon-LB，以具有[必要权限的用户身份登录到 DC/OS Web 界面](/dcos/cn/1.11/security/ent/perms-reference/)。
...
- Marathon-LB 需要一个`strict` [安全模式](/dcos/cn/1.11/installing/ent/custom/configuration/configuration-parameters/#security-enterprise)中的服务帐户。只有具有 `superuser` 权限的用户才能创建服务帐户。请参阅 [Provisioning Marathon-LB](/dcos/cn/services/marathon-lb/mlb-auth/) 了解说明。

- [已安装 DC/OS CLI](/dcos/cn/1.11/cli/install/) 并作为具有 [必要权限](/dcos/cn/1.11/security/ent/perms-reference/)的用户通过 `dcos auth login` 登录。

需要 [Provisioning Marathon-LB](/dcos/cn/services/marathon-lb/mlb-auth/) 中显示的参数才能安装 Marathon-LB。在安装服务之前，您可能希望修改其他默认值。要查看 Marathon-LB 的配置选项和默认值，请键入以下命令。
