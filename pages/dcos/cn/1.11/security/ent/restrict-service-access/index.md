...
本教程演示如何在宽容[安全模式](/dcos/cn/1.11/security/ent/#security-modes)下实现 DC/OS 服务的用户权限。完成后，您将通过使用 DC/OS 权限获得多租户。

**先决条件：**

- DC/OS Enterprise [已安装](/dcos/cn/1.11/installing/ent/)在宽容[模式](/dcos/cn/1.11/security/ent/#security-modes)下。

## 创建用户和组

1. 从 **Services > Services > Create Group** 创建服务组。

    ![服务创建组](/dcos/cn/1.11/img/service-group1.png)

    图 1. 创建组页面

    在此示例中，创建了一个名为 `prod-a` 和一个名为 `prod-b`。创建组之后，您应该看到两个文件夹。您可以在此处为用户组部署服务并为每个单元设置权限。

    ![组文件夹](/dcos/cn/1.11/img/service-group2.png)
...
![service-group4](/dcos/cn/1.11/img/service-group4.png)
...
接下来，我们将创建组并为 DC/OS 服务分配权限。
 ...
 ![prod-a group](/dcos/cn/1.11/img/service-group5.png)
...
1. 为 prod-a-group 添加这些权限，然后单击 **Close**。
    ...
    ![prod-a-group](/dcos/cn/1.11/img/service-group7.png)

    图 6. 为“prod-a-group”添加权限

    以下是添加后的权限视图：

    ![prod-a-group](/dcos/cn/1.11/img/service-group8.png)
...
1. 选择 **Group Membership**，然后在搜索框中键入 `prod-agroup`，然后单击选择。
    ...
    ![prod-a-group](/dcos/cn/1.11/img/service-group9.png)
...
1. 以 Cory 身份登录到 DC/OS Web 界面。您可以看到，用户 Cory 只能访问 **Services** 和 **Universe** 选项卡。同时，Cory 只能看到 **prod-a** 服务。
  ...
  ![prod-a-group](/dcos/cn/1.11/img/service-group10.png)
...
1. 选择 **JSON Configuration** 并粘贴到以下应用定义中：
      ...
      ![JSON 视图](/dcos/cn/1.11/img/service-group15.png)
...
1. 在 Cory 或 Nick 登录时，单击 NGINX 启动图标，查看确认消息。
    ...
    ![NGINX](/dcos/cn/1.11/img/service-group-nginx.png)
...
1. 注销当前用户，然后以具有[超级用户](/dcos/cn/1.11/security/ent/perms-reference/#superuser)权限的用户身份重新登入。您将看到，两个服务都在 prod-a 和 prod-b 组中运行。

  ![prod-a-group](/dcos/cn/1.11/img/service-group14.png)
