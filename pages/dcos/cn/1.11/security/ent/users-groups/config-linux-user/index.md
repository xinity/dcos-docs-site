...
服务或作业的默认 Linux 用户可能因安全模式和容器类型而异 请参阅 [Linux 用户](/dcos/cn/1.11/security/ent/#linux-users) 了解更多信息。
...
许多 Universe 服务忽略其用户帐户的覆盖，在 `strict` 模式下除外。我们提供覆盖服务默认 Linux 用户的详细步骤，该服务在[服务账户](/dcos/cn/1.11/security/ent/service-auth/)中支持此覆盖。有关逐步说明，请参阅与感兴趣的服务相关的部分。程序还包括如何配置服务以使用加密和服务帐户。
...
- 代理程序上已存在 Linux 用户帐户。
- 您已安装并登录到 [DC/OS CLI](/dcos/cn/1.11/cli/)。
...
1. 使用 [Marathon API](/dcos/cn/1.11/deploying-services/marathon-api/) 部署服务。
...
- 代理程序上已存在 Linux 用户帐户。
- 您已安装并登录到 [DC/OS CLI](/dcos/cn/1.11/cli/)。
- 如果您的 [安全模式](/dcos/cn/1.11/security/ent/#security-modes) 是 `permissive` 或 `strict`，则必须遵守 [下载根证书](/dcos/cn/1.11/security/ent/tls-ssl/get-cert/) 中的步骤才能发布此部分的 curl 命令。如果您的[安全模式](/dcos/cn/1.11/security/ent/#security-modes)为 `disabled`，则必须在将其发出前从命令中删除 `--cacert dcos-ca.crt`。
