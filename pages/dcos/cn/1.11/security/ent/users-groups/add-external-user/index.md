...
- 外部 [LDAP 目录](/dcos/cn/1.11/security/ent/ldap/)。
- [OpenID Connect 或 SAML 提供程序](/dcos/cn/1.11/security/ent/sso/)。
...
1. 完成后，对话框应如下所示。
    ...
    ![LDAP 组导入配置](/dcos/cn/1.11/img/ldap-group-import.png)
...
您可以使用 `/ldap/importuser` [IAM API](/dcos/cn/1.11/security/ent/iam-api/) 端点导入一组 LDAP 用户。
...
- 如果您[安全模式](/dcos/cn/1.11/security/ent/#security-modes)为 `permissive` 或 `strict`，则在本部分中发出 curl 命令之前，必须遵循[获取 DC/OS CA 根证书]中的步骤(/dcos/cn/1.11/security/ent/tls-ssl/get-cert/)。如果您的[安全模式](/dcos/cn/1.11/security/ent/#security-modes)为 `disabled`，则必须在将其发出前从命令中删除 `--cacert dcos-ca.crt`。
