...
This section continues on from the instructions given in the [Basic Installation](/dcos/services/kubernetes/2.1.0-1.12.3/getting-started/provision-install/) section.
...
See the [Cluster Sizing](/dcos/services/kubernetes/2.1.0-1.12.3/operations/cluster-sizing/) page for a more detailed explanation.
...
It is possible that the region may not have the resources required, make sure to read and understand [Cluster Sizing](/dcos/services/kubernetes/2.1.0-1.12.3/operations/cluster-sizing/) before attempting a deployment.
...
**NOTE:** Multi-Region deployment is not supported, see [Limitations](/dcos/services/kubernetes/2.1.0-1.12.3/limitations/index/).
...
* [DC/OS CA](/dcos/1.12/security/ent/tls-ssl/) - a centralized certificate-authority (CA) for validating and, eventually, signing certificate signing requests (CSRs).
* [DC/OS Secrets](/dcos/1.12/security/ent/secrets/) - a centralized and secure way to distribute TLS artifacts to package components, such as the Kubernetes components, and other applications living in the same DC/OS cluster.
* [DC/OS Service Accounts](/dcos/1.12/security/ent/service-auth/) - needed for our package and applications to authenticate against the services named above.

DC/OS does not provide the functionality describe above. The following diagram describes how this was achieved on DC/OS.

![alt text](/dcos/services/kubernetes/2.1.0-1.12.3/img/tls.png "TLS design")

Figure 1. TLS Design

### TLS Provisioning in DC/OS Enterprise

When installing DC/OS Kubernetes on a DC/OS Enterprise cluster, a [service account](/dcos/1.12/security/ent/service-auth/) is mandatory.
...
1. In order to provision such service account, first you need to install the [DC/OS Enterprise CLI](/dcos/1.12/cli/). Then, run the following:
    ...
    ```
1. Next, you need to [grant](/dcos/1.12/security/ent/perms-management/) the service account the correct [permissions](/dcos/1.12/security/ent/perms-reference/). The required permissions are:
