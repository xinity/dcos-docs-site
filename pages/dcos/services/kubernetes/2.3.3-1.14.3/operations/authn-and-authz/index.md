...
![alt text](/dcos/services/kubernetes/2.3.3-1.14.3/img/authorization-mode.png "Authorization Mode")
...
Once [kubectl is setup for the bootstrap user](/dcos/services/kubernetes/2.3.3-1.14.3/operations/connecting-clients/) you may wish to grant other users access to specific resources running in the cluster.
...
To retrieve the generated token for `my-pod-sa` service account that can later be used to configure `kubectl` or access the [Kubernetes Dashboard](/dcos/services/kubernetes/2.3.3-1.14.3/operations/kubernetes-dashboard/), get the secret name and then that secret's token:
