...
To submit a {{ model.techShortName }} job inside the [DC/OS Overlay Network](/dcos/1.12/overview/design/overlay/), run a command similar to the following:
...
Note that DC/OS overlay support requires the [UCR](/dcos/1.12/deploying-services/containerizers/ucr/)   rather than the default Docker Containerizer, so you must set `--conf spark.mesos.containerizer=mesos`.
...
The default DC/OS {{ model.techName }} distribution is compiled against Hadoop 2.9 libraries.  However, you can choose a different version by following the instructions in [Customize {{ model.techShortName }} distribution](/dcos/services/spark/2.8.0-2.4.0/install/#custom-dist/).
