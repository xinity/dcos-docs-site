...
* [kubectl][install_kubectl] v1.16.5 or later
...
1. Run the following commands to initialize Konvoy in the current working directory:
   ...
   ```bash
   konvoy init --provisioner=none --addons-config-repository /opt/konvoy/artifacts/kubernetes-base-addons --addons-config-version v1.3.0-rc.1 [--cluster-name <your-specified-name>]
   ```
...
1. During regular deployment your cluster would have access to publicly hosted Helm chart repos for all of the addons.
   ...
   ```yaml
     ...
     - configRepository: /opt/konvoy/artifacts/kubernetes-base-addons
       configVersion: v1.3.0-rc.1
       helmRepository:
         image: mesosphere/base-addons-chart-repo:v1.3.0-rc.1
   ```
