...
<p class="message--note"><strong>NOTE: </strong>The <code>cn dn node identity</code> parameter specifies the initial node identity for TLS, which is used inside <code>authorizers.xml</code> file for CN value.</p>
...
Sample JSON options file named `sample-{{ model.packageName }}-custom.json`:
   ...
   ```
The following command creates a cluster using `sample-{{ model.packageName }}-custom.json`:
   ```shell
